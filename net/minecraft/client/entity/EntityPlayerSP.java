...
import ru.terracotta.rikardo.utils.Rotation;
...
public class EntityPlayerSP extends AbstractClientPlayer
{
    ...
    private void onUpdateWalkingPlayer()
    {
        ...
        if (this.isCurrentViewEntity())
        {
            ...
            if (!Main.moduleManager.getModule("FreeCam").isEnable()) {
                float r = Rotation.getYaw();
                float b = Rotation.getPitch();
                mc.player.rotationYawHead = r;
                if (this.isRiding())
                {
                    this.connection.sendPacket(new CPacketPlayer.PositionRotation(this.motionX, -999.0D, this.motionZ, r, b, this.onGround));
                    flag2 = false;
                }
                else if (flag2 && flag3)
                {
                    this.connection.sendPacket(new CPacketPlayer.PositionRotation(this.posX, axisalignedbb.minY, this.posZ, r, b, this.onGround));
                }
                else if (flag2)
                {
                ...
                {
                    this.connection.sendPacket(new CPacketPlayer.Rotation(r, b, this.onGround));
                }
    ...
    {
        if (Main.moduleManager.getModule("FreeCam").isEnable() || this.noClip) {
            return false;
        } else {
